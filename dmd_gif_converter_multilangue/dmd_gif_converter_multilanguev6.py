...
class ManualEffects:
    ...
    def flash_effect(img, duration=1.0, fps=10, flashes=3):
        ...
        for i in range(num_frames):
            ...
            canvas = Image.new(
                "RGB", (128, 32), (255, 255, 255) if is_flash else (0, 0, 0)
            )
            if not is_flash:
                w, h = img.size
...
class DMDConverter:
    ...
    def __init__(self):
        ...
        self.theme.trace_add(
            "write", lambda *args: config_manager.set("theme", self.theme.get())
        )
        self.theme.trace_add(
            "write", lambda *args: config_manager.set("theme", self.theme.get())
        )
        self.add_anim_to_name = tk.BooleanVar(
            value=config_manager.get("add_anim_to_name", True)
        )
        ...
        def update_recursive(widget):
            try:
                ...
                if widget_class in ("TButton", "Button", "TLabel", "Label"):
                    ...
                    if current in text_map:
                        key = text_map[current]
    ...
    def setup_ui(self):
        ...
        ttk.Label(header, text="DMD Converter v2.0", font=("Arial", 12, "bold")).pack(
            side=tk.LEFT
        )
        ...
        self.progressbar.grid(row=1, column=0, columnspan=3, sticky="ew", padx=10, pady=5)
    ...
    def setup_auto_tab(self):
        ...
        proposals_frame.pack(fill=tk.BOTH, expand=False)
        ...
        for i in range(5):
            ...
            canvas = Canvas(
                frame, width=256, height=64, cursor="hand2", bg="black"
            )  # valeur d origine 256 reduite pour depassemnt de fenetre en attendant mieux
            canvas.pack()
        ...
        ttk.Button(action_frame, text="✅ Traiter sélection", command=self.process_selected).pack(
            side=tk.LEFT, padx=5, pady=2
        )
        tk.Button(
            action_frame,
        ...
        content_frame.columnconfigure(1, weight=3)  # Centre (previews + propositions IA)
    ...
    def setup_manual_tab(self):
        ...
        info_image_frame = ttk.LabelFrame(
            right_panel, text="ℹ️ Informations Image", padding="5"
        )
    ...
    def setup_textscroll_tab(self):
        ...
        ttk.Checkbutton(
            row2, text="Auto-ajuster", variable=tk.BooleanVar(value=True)
        ...
        preview_frame = ttk.LabelFrame(
            right_panel, text="Aperçu Animation", padding="5"
        )
        preview_frame.pack(fill=tk.BOTH, expand=False)
    ...
    def show_original(self, image_path):
        ...
        if img_orig.mode in ("RGBA", "LA") or img_orig.info.get("transparency") is not None:
...
"""
    ...
    def auto_analyze_and_preview(self, image_path):
        ...
        try:
            ...
            self.ia_status_var.set(
                f"✓ Meilleure: '{self.proposals[0][0]['name']}' (score: {scored_variants[0][0]:.1f})"
            )
    ...
    def evaluate_quality(self, original_img, dmd_canvas):
        ...
        orig_resized = original_img.convert("RGB").resize(
            (128, 32), Image.Resampling.LANCZOS
        )
    ...
    def render_dmd_frame(self, image_path, settings, return_frames=False):
        ...
        img, new_w, new_h = DMDEngine.adaptive_resize(
            img, 128, 32, settings["resize_mode"]
        )
    ...
    def process_images(self, image_list):
        ...
        if self.images:
            ...
            messagebox.showwarning("Erreur", "Aucune image chargée pour définir le dossier source")
            return
        ...
        for i, img_path in enumerate(to_process):
            ...
            try:
                ...
                frames, fps = self.render_dmd_frame(
                    img_path, settings, return_frames=True
                )
                ...
                p_frames[0].save(
                    ...
                    loop=(
                        0
                        if self.manual_loop_mode.get() == "infini"
                        else self.manual_loop_count.get()
                    ),
                    optimize=True,
                )
                logger.info(
        ...
        if not self.processing_canceled:
            self.root.after(0, lambda: self.clear_progress_and_notify(total, output_dir))
