...
class ConfigManager:
    ...
    def __init__(self):
        ...
        if appdata:
            self.user_dir = Path(appdata) / "DMD_GIF_Converter_modulaire_base"
        else:
            self.user_dir = (
                Path.home() / "AppData" / "Roaming" / "DMD_GIF_Converter_modulaire_base"
            )
...
"""
    ...
    def process_images(self, image_list):
        ...
        if not self.processing_canceled:
            self.root.after(
                0, lambda: self.clear_progress_and_notify(total, output_dir)
            )
